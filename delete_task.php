...
{
    $id_task = $_GET['delete_id'];
    $dbfun->deleteTask($id_task);
    header("Location: delete_task.php?deleted"); 
}
?>
...
<div class="container hero-unit">
     ...
     if(isset($_GET['deleted']))
    {
        ...
        <strong>Tak!</strong> Zadanie usunięte. <a href="tasks.php">Wróć</a> 
        </div>
        <?php
    }
    else
    {
     ...
     if(isset($_GET['delete_id']))
     {
         ...
         <?php
         $stmt = $db->prepare("SELECT * FROM tasks WHERE id_task=:id_task");
         $stmt->execute(array(":id_task"=>$_GET['delete_id']));
...
if(isset($_GET['delete_id']))
{
    ...
    <tr>
    <form method="post">
        <input type="hidden" name="id" value="<?php echo $row['id_task']; ?>" />
